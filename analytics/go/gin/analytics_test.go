...
func setupGinTest(t *testing.T, config *Config) (*gin.Engine, *MockAnalyticsServer, *core.Client) {
	...
	return r, mockServer, client
}

func TestGinAnalyticsMiddleware(t *testing.T) {
	r, mockServer, client := setupGinTest(t, nil)
...
func TestGinPanicRecovery(t *testing.T) {
	...
	// unwinds through a panic is not recorded; gin.Recovery() turning the panic
	// into a 500 response (asserted above) is what this test verifies.
}
...
func TestGinCustomConfig(t *testing.T) {
	...
	r, mockServer, client := setupGinTest(t, customConfig)
...
func TestGinPrivacyLevels(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			r, mockServer, client := setupGinTest(t, config)
