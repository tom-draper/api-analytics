...
[![npm](https://img.shields.io/npm/v/@api-analytics/nestjs)](https://www.npmjs.com/package/@api-analytics/nestjs)

```bash
npm install @api-analytics/nestjs
```

Apply the middleware globally in `main.ts`:

```ts
import { NestFactory } from '@nestjs/core';
import { AppModule } from './app.module';
import { nestjsAnalytics } from '@api-analytics/nestjs';

async function bootstrap() {
    const app = await NestFactory.create(AppModule);

    app.use(nestjsAnalytics('YOUR-API-KEY'));  // Add middleware
...
```ts
import { nestjsAnalytics, Config } from '@api-analytics/nestjs';

const config = new Config();
config.getIPAddress = (req) => req.headers['x-forwarded-for'];
config.getUserAgent = (req) => req.headers['user-agent'];
config.getUserID = (req) => req.headers['x-auth-token'] ?? null;

app.use(nestjsAnalytics('YOUR-API-KEY', config));  // Add middleware
```
