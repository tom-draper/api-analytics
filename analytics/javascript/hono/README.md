...
[![npm](https://img.shields.io/npm/v/@api-analytics/hono)](https://www.npmjs.com/package/@api-analytics/hono)
...
app.use('*', honoAnalytics('YOUR-API-KEY'));  // Add middleware
...
```js
import { Hono } from 'hono';
import { honoAnalytics, Config } from '@api-analytics/hono';

const config = new Config();
config.getUserID = (req) => req.header('x-auth-token') ?? null;

const app = new Hono();

app.use('*', honoAnalytics('YOUR-API-KEY', config));  // Add middleware
```
