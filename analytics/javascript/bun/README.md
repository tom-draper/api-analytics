...
[![npm](https://img.shields.io/npm/v/@api-analytics/bun)](https://www.npmjs.com/package/@api-analytics/bun)

```bash
bun add @api-analytics/bun
```

```js
import { bunAnalytics } from '@api-analytics/bun';
...
```js
import { bunAnalytics, Config } from '@api-analytics/bun';

const config = new Config();
config.getUserID = (req) => req.headers.get('x-auth-token') ?? null;
