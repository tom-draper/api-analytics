from dataclasses import dataclass
from datetime import datetime
from time import time
from typing import Callable, Union

import falcon

from .core import log_request, logger, DEFAULT_SERVER_URL


class Analytics:
    """API Analytics middleware for Falcon."""

    def __init__(self, api_key: str, config: "Config" = None):
        self.api_key = api_key
        self.config = config or Config()

        if not self.api_key:
            logger.debug("API key is not set.")
        if not self.config.server_url:
            logger.debug("Server URL is not set.")

    def process_request(self, req: falcon.Request, resp: falcon.Response):
        req.context.analytics_start = time()

    def process_response(
        self,
        req: falcon.Request,
        resp: falcon.Response,
        resource,
        req_succeeded: bool,
    ):
        start = getattr(req.context, "analytics_start", time())
        status = resp.status if isinstance(resp.status, int) else int(str(resp.status)[:3])

        request_data = {
            "hostname": self.config.get_hostname(req),
            "ip_address": self._get_ip_address(req),
            "path": self.config.get_path(req),
            "user_agent": self.config.get_user_agent(req),
            "method": req.method,
            "status": status,
            "response_time": int((time() - start) * 1000),
            "user_id": self.config.get_user_id(req),
            "created_at": datetime.now().isoformat(),
        }

        log_request(
            self.api_key,
            request_data,
            "Falcon",
            self.config.privacy_level,
            self.config.server_url,
        )

    def _get_ip_address(self, req: falcon.Request) -> Union[str, None]:
        if self.config.privacy_level >= 2:
            return None
        return self.config.get_ip_address(req)


class Mappers:
    @staticmethod
    def get_path(req: falcon.Request) -> Union[str, None]:
        return req.path

    @staticmethod
    def get_ip_address(req: falcon.Request) -> Union[str, None]:
        return req.remote_addr

    @staticmethod
    def get_hostname(req: falcon.Request) -> Union[str, None]:
        return req.host

    @staticmethod
    def get_user_id(req: falcon.Request) -> Union[str, None]:
        return None

    @staticmethod
    def get_user_agent(req: falcon.Request) -> Union[str, None]:
        return req.get_header("user-agent")


@dataclass
class Config:
    """
    Configuration for the Falcon API Analytics middleware.

    :param privacy_level: Controls client identification by IP address.
        - 0: Sends client IP to the server to be stored and client location is
        inferred.
        - 1: Sends the client IP to the server only for the location to be
        inferred and stored, with the IP discarded afterwards.
        - 2: Avoids sending the client IP address to the server. Providing a
        custom `get_user_id` mapping function becomes the only method for client
        identification.
        Defaults to 0.
    :param server_url: For self-hosting. Points to the public server url to post
        requests to.
    :param get_path: Mapping function that takes a request and returns the path
        stored within the request. Assigning a value will override the default
        behavior.
    :param get_ip_address: Mapping function that takes a request and returns the
        IP address stored within the request. Assigning a value will override the
        default behavior.
    :param get_hostname: Mapping function that takes a request and returns the
        hostname stored within the request. Assigning a value will override the
        default behavior.
    :param get_user_agent: Mapping function that takes a request and returns the
        user agent stored within the request. Assigning a value will override the
        default behavior.
    :param get_user_id: Mapping function that takes a request and returns a
        custom user ID stored within the request. Always returns None by default.
        Assigning a value allows for tracking a custom user ID specific to your API
        such as an API key or client ID. If left as the default value, user
        identification may rely on client IP address only (depending on
        `privacy_level`).
    """

    privacy_level: int = 0
    server_url: str = DEFAULT_SERVER_URL
    get_path: Callable[[falcon.Request], Union[str, None]] = Mappers.get_path
    get_ip_address: Callable[[falcon.Request], Union[str, None]] = Mappers.get_ip_address
    get_hostname: Callable[[falcon.Request], Union[str, None]] = Mappers.get_hostname
    get_user_agent: Callable[[falcon.Request], Union[str, None]] = Mappers.get_user_agent
    get_user_id: Callable[[falcon.Request], Union[str, None]] = Mappers.get_user_id
