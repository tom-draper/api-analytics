...
def add_middleware(app: web.Application, api_key: str, config: "Config" = None):
    """
    Adds API Analytics middleware to the aiohttp app to log requests to the server.

    :param app: aiohttp Application to attach the analytics middleware to
    ...
    @web.middleware
    async def analytics_middleware(request: web.Request, handler) -> web.Response:
        start = time()
        ...
        log_request(
            api_key, request_data, "aiohttp", config.privacy_level, config.server_url
        )
        return response

    app.middlewares.insert(0, analytics_middleware)


class Mappers:
    @staticmethod
    def get_path(request: web.Request) -> Union[str, None]:
        return request.path

    @staticmethod
    def get_ip_address(request: web.Request) -> Union[str, None]:
        return request.remote

    @staticmethod
    def get_hostname(request: web.Request) -> Union[str, None]:
        return request.host

    @staticmethod
    def get_user_id(request: web.Request) -> Union[str, None]:
        return None

    @staticmethod
    def get_user_agent(request: web.Request) -> Union[str, None]:
        return request.headers.get("User-Agent")


def _get_ip_address(request: web.Request, config: "Config") -> Union[str, None]:
...
class Config:
    """
    Configuration for the aiohttp API Analytics middleware.
    ...
    get_path: Callable[[web.Request], Union[str, None]] = Mappers.get_path
    get_ip_address: Callable[[web.Request], Union[str, None]] = Mappers.get_ip_address
    get_hostname: Callable[[web.Request], Union[str, None]] = Mappers.get_hostname
    get_user_agent: Callable[[web.Request], Union[str, None]] = Mappers.get_user_agent
    get_user_id: Callable[[web.Request], Union[str, None]] = Mappers.get_user_id
