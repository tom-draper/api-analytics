...
if [ $# -ge 1 ]; then
    ...
    echo "Testing internal API key generation via Nginx..."

    response=$(curl -skL -X GET -w "%{http_code}" http://localhost/api/generate)
...
echo "Testing internal request data logging via Nginx..."

response=$(curl -skLo - -w "%{http_code}" -X POST  \
    ...
    http://localhost/api/log-request)
...
echo "Testing internal user ID access via Nginx..."

response=$(curl -skL -X GET -w "%{http_code}" http://localhost/api/user-id/$api_key)
echo $response
status_code="${response: -3}"   # Extract the last 3 characters as the status code
response_body="${response%???}"
...
echo "Testing internal dashboard data access via Nginx..."

response=$(curl -skL -X GET -w "%{http_code}" --compressed -H "Accept-Encoding: gzip" \
    http://localhost/api/requests/$user_id)
...
echo "Testing internal raw data access via Nginx..."

response=$(curl -skL -X GET -w "%{http_code}" \
    http://localhost/api/data --header "X-AUTH-TOKEN: $api_key")
...
echo "Testing internal account deletion va Nginx..."

response=$(curl -skL -X GET -w "%{http_code}" http://localhost/api/delete/$api_key)
