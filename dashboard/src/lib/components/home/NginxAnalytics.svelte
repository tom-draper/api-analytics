<script lang="ts">
</script>

<div class="flex w-full justify-center py-16 pb-18">
	<div class="container mx-auto max-w-4xl px-6">
		<div
			class="overflow-hidden rounded border border-neutral-800 border-[#2e2e2e] bg-neutral-900 bg-[#181818] shadow-lg transition-all hover:shadow-xl"
		>
			<div class="flex flex-col items-center gap-6 p-6 md:flex-row">
				<div class="shrink-0">
					<img src="images/logos/nginx-analytics.svg" alt="Nginx Analytics" class="h-24 w-auto" />
				</div>
				<div class="flex-1 text-center md:text-left">
					<h2 class="mb-2 text-xl font-bold text-white">Using Nginx?</h2>
					<p class="mb-4 text-neutral-300 p-0">
						Try <span class="font-medium text-[var(--highlight)]">Nginx Analytics</span>, a flexible and
						fully self-hosted solution for real-time monitoring.
					</p>
					<a
						href="https://github.com/tom-draper/nginx-analytics"
						target="_blank"
						class="inline-flex items-center rounded bg-[var(--highlight)] px-5 py-2 font-medium text-neutral-900 transition-colors hover:bg-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-500"
					>
						Get Started
						<svg
							xmlns="http://www.w3.org/2000/svg"
							class="ml-2 h-5 w-5"
							viewBox="0 0 20 20"
							fill="currentColor"
						>
							<path
								fill-rule="evenodd"
								d="M10.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L12.586 11H5a1 1 0 110-2h7.586l-2.293-2.293a1 1 0 010-1.414z"
								clip-rule="evenodd"
							/>
						</svg>
					</a>
				</div>
			</div>
		</div>
	</div>
</div>
